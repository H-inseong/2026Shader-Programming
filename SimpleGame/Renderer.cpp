...
void Renderer::Initialize(int windowSizeX, int windowSizeY)
{
	...
	m_BlurHShader	= CompileShaders("./Shaders/BlurH.vs", "./Shaders/BlurH.fs");
	m_BlurVShader	= CompileShaders("./Shaders/BlurV.vs", "./Shaders/BlurV.fs");
...
void Renderer::GenFBOs()
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		if (status != GL_FRAMEBUFFER_COMPLETE)
		{
			std::cout << "Pingpong FBO creation failed. Status: " << status << std::endl;
			assert(0);
		}
	}
...
void Renderer::DrawFSShader()
{
	...
	int attribTexCoord = glGetAttribLocation(m_FSShader, "a_Tex");
